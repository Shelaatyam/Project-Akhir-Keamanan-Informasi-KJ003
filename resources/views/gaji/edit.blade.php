    ...
    <div class="container">
        ...
        <form action="{{ route('gaji.update', $gaji->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="karyawan_id" name="karyawan_id" required>
                    @foreach ($karyawans as $karyawan)
                        <option value="{{ $karyawan->id }}" @if ($karyawan->id == $gaji->karyawan_id) selected @endif>
                            {{ $karyawan->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control" id="tunjangan" name="tunjangan" value="{{ $gaji->tunjangan }}" required>
            </div>
            <div class="mb-3">
                <label for="total_gaji" class="form-label">Total Gaji</label>
                <input type="number" class="form-control" id="total_gaji" name="total_gaji" value="{{ $gaji->total_gaji }}" required>
            </div>
